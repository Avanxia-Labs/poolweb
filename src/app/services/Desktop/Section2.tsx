...
//         <div className=" flex flex-col w-full md:items-center lg:w-2/5 gap-8 md:gap-10 lg:gap-12">
//           {/* Services Column - Mismo ancho que el calculador */}
//           <div id='Services' className="xl:mt-[-250px] w-full mt-6 bg-white rounded-lg shadow-sm" style={{minWidth: '30rem', maxWidth: '30rem'}}>
...
const Section2 = () => {
  ...
  const [currentComponent, setCurrentComponent] = useState<'input' | 'explore'>(
    ['pool-cleaning', 'pool-maintenance'].includes('pool-cleaning') ? 'input' : 'explore'
  );

  // Encontrar el servicio actualmente seleccionado
  const currentService = servicesData.find(service => service.id === selectedService) || servicesData[0];

  // Efecto para manejar la transición cuando cambia el servicio seleccionado
  useEffect(() => {
    const shouldShowInput = ['pool-cleaning', 'pool-maintenance'].includes(selectedService);
    const newComponent = shouldShowInput ? 'input' : 'explore';
    
    if (currentComponent !== newComponent) {
      // Iniciar transición de salida
      setVisible(false);
      
      // Después de que termine la transición de salida, cambiar el componente
      const timer = setTimeout(() => {
        setCurrentComponent(newComponent);
        // Iniciar transición de entrada
        setVisible(true);
      }, 350); // tiempo de transición de salida (debe coincidir con la duración de la transición CSS)
  ...
  return (
    // Contenedor exterior que permite que el fondo se extienda a todo el ancho disponible
    <div className="w-full bg-[#F5F9FF]">
      {/* Contenedor interior con estructura grid */}
      <div className="grid grid-cols-1 xl:grid-cols-12 w-full py-8 md:py-12 xl:py-16 px-4 md:px-8 xl:px-16 xl:px-32 
        ...
        <div className="xl:col-span-5 w-full flex flex-col items-center gap-8 md:gap-10 xl:gap-14">
          ...
          {/* Segundo componente: Calculadora o ExplorePoolServices con transición */}
          <div className="w-full flex justify-center">
            <div 
            ...
            >
              {currentComponent === 'input' ? <InputDesign /> : <ExplorePoolServices />}
            </div>
          </div>
        </div>
      </div>
