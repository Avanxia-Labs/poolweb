    ...
    <div className='flex w-full h-full flex-col bg-white p-6 rounded-2xl shadow-lg max-w-[400px]'>
      <div className='flex items-center gap-4 mb-4'>
        <img 
          src={avatar} 
          alt={`${name}'s avatar`} 
          className='flex p-[10px_10px_10px_0px] justify-center items-center gap-[10px] rounded-full object-cover'
        />
        <div>
          <p className='text-[#18181B] font-plus-jakarta font-bold text-base leading-7'>{name}</p>
          <p className='text-[#18181B] font-plus_jakarta_sans text-[14px] not-italic font-normal leading-[22px]'>{profession}</p>
      ...
      <div className="flex-grow overflow-hidden relative">
        <p className="text-[#18181B] font-plus_jakarta_sans text-[18px] font-normal leading-[28px] mt-[10px] 
                      overflow-y-auto max-h-[160px] sm:max-h-[100px] md:max-h-[140px] lg:max-h-[180px] 
                      pr-2 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent 
                      touch-auto">
          {description}
        </p>
      </div>
