...
const MobileMenu = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const getLinkClass = (href: string) =>
    `flex items-center gap-2 text-[14px] leading-[22px] font-medium tracking-[0.28px] font-inter ${pathname === href
  ...
  return (
    <div
      className={`fixed top-0 right-0 z-[100] h-full bg-white shadow-lg w-3/4 max-w-xs flex flex-col justify-between transition-transform duration-300 ${isOpen ? 'translate-x-0' : 'translate-x-full'
        }`}
    >
      {/* Top */}
      <div className="pt-[120px] px-6">
        ...
        <nav className="flex flex-col gap-[38px] items-start w-full">
          ...
          <Link href="/contact" onClick={onClose} className="mt-[30px] w-full flex justify-center">
            <button
              className={`w-[212px] px-[20px] py-[10px] flex items-start justify-center gap-[21px] rounded-none text-[14px] leading-[22px] font-medium tracking-[0.28px] font-inter transition-colors duration-300 ${pathname === '/contact' ? 'bg-[#3A48CC]' : 'bg-[#485AFF]'
                } text-white`}
            >
