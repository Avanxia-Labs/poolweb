...
import ConstructionGalleryMobile from './ConstructionGalleryMobile';
import CarouselPagination from '@/components/shared/CarouselPagination';

// Services reordered - Custom Pool Design & Construction now in position 2 (middle)
const services = [
  {
    id: 'pool-maintenance',
  ...
  },
  {
    ...
    longText: 'Keep your pool in peak condition with Pool Quality\'s expert diagnostics and troubleshooting. We specialize in identifying and resolving any issues affecting your pool\'s functionality. From equipment malfunctions to water chemistry imbalances, our skilled technicians employ advanced techniques to pinpoint the root cause of problems. Our comprehensive diagnostic process includes thorough inspections, precise testing, and detailed analysis. We address issues such as leaks, pump failures, filtration inefficiencies, and heating system faults. By providing accurate assessments and effective solutions, we minimize downtime and prevent further damage. Trust Pool Quality to restore your pool to its optimal state. We offer clear explanations of our findings, transparent recommendations for repair or maintenance, and reliable service to get you back to enjoying your pool as quickly as possible.',
...
const MobileHeroContent = () => {
  ...
  // Get initial service index from URL parameter
  const getInitialServiceIndex = () => {
    const serviceParam = searchParams.get('service');
    if (serviceParam) {
      const index = services.findIndex(s => s.id === serviceParam);
      if (index !== -1) return index;
    }
    return 2; // Default to Custom Pool Design & Construction
  };

  const initialIndex = getInitialServiceIndex();
  const [currentServiceTitle, setCurrentServiceTitle] = useState(services[initialIndex].title);
  ...
  const [activeServiceIndex, setActiveServiceIndex] = useState(initialIndex);
  ...
  useEffect(() => {
    ...
    if (serviceParam) {
      ...
      if (serviceIndex !== -1) {
        ...
        setCurrentServiceTitle(services[serviceIndex].title);
      }
    }
  }, [searchParams]);
    ...
    <main>
      <section className="relative w-full min-h-[500px] overflow-hidden">
        ...
        <div className="relative z-20 pt-[100px] flex flex-col items-center">
          ...
          >
            <h1 className="text-3xl text-black text-center font-['Plus_Jakarta_Sans'] font-bold leading-tight tracking-tight">
              {services[index].title}
            </h1>
            <p
              className="text-base font-medium text-white text-center leading-relaxed font-inter"
              style={{
        ...
        <div className="absolute bottom-0 w-full bg-white border-y border-[#E5E7EB] z-20">
          {/* Container with scroll indicators */}
          <div className="relative">
            {/* Left fade gradient indicator */}
            <div className="tabs-scroll-indicator-left pointer-events-none" />
            {/* Right fade gradient indicator */}
            <div className="tabs-scroll-indicator-right pointer-events-none" />
              ...
              <div className="flex gap-[17px] px-4 py-3 min-w-fit w-max">
                {services.map((service, index) => (
                  <button
                    ...
                    className={`flex items-center h-[46px] px-[20px] py-[14px] gap-2 font-inter text-sm transition-colors whitespace-nowrap ${service.id === 'design-and-construction' ? 'font-semibold' : 'font-normal'
                  ...
                  >
                    {service.id === 'design-and-construction' && (
                      <span className={`text-sm ${activeServiceIndex === index ? 'text-yellow-300' : 'text-yellow-500'}`}>&#9733;</span>
                    )}
                    {service.title}
                    {service.id === 'design-and-construction' && (
                      <span className={`ml-1 text-[10px] px-1.5 py-0.5 rounded-full ${activeServiceIndex === index
      ...
      {services[activeServiceIndex] && (
        services[activeServiceIndex].id === 'design-and-construction' ? (
          <section className="w-full pt-6 bg-[#F7FAFE]">
            <ConstructionGalleryMobile />
          </section>
        ) : (
          <section
            key={activeServiceIndex}
            className="w-full px-4 pt-10 pb-8 bg-[#F7FAFE] text-[#212939] text-sm leading-relaxed"
          >
            <h2 className="text-2xl font-bold text-[#3E57DA] text-center capitalize font-['Plus_Jakarta_Sans']">
            ...
            <p
              className={`text-justify pt-12 mb-4 text-sm text-[#0B0858] leading-relaxed font-normal font-inter normal-case overflow-hidden text-ellipsis ${showFullText ? '' : 'line-clamp-[12]'
                }`}
            >
